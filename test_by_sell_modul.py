...
class balance():
    def __init__(self, data_table: pd.DataFrame):
        ...
        self.porog_by = 0.2  # Пороговое значение для покупки и продажи
        self._dey_changer()
    ...
    def _dey_changer(self):
        ...
        for day in self.data_tb.index[0:-1:1]:
            ...
            try:
                ...
                print(self.data_tb.loc[self.__next_day])
                if self.data_tb.loc[self.__next_day, 'Open'] > 0 and self.__next_day > datetime.date(2021, 6,
                                                                                                     10):  # Установить дату откуда считать
                    self._sell_by()
            except KeyError:
                None
...
def main(asset, znach_find):
    ...
    for n in ([8]):  # "Периоды таймфреймов
        ...
        for x1, x2, x3 in znach_find:
            ...
            print(index)
            print(znach_doh)
            final = final.append({"n": n, "Параметры wk": index, "Баланс": znach_doh, "Шарпа": sharp},
                                 ignore_index=True)
            print(final)
...
if __name__ == "__main__":
    ...
    asset = "HBAR"
    date_tb = graf_delta_cls(720, asset)
    analiz = pd.DataFrame()
