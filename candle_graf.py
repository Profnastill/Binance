...
def fun_graf_delta(asset, day_interval,day=None):
    ...
    if candel_tb["Open time"][-1::].values==np.datetime64(tcur_time-timedelta(3)):# Отсеиваем элементы для которых по какой то причине данные не сооответствуют тек дню
        return
    ...
    for i in range(len(n1)):
        ...
        wk = {0: 0.45, 1: 0.35, 2: 0.2}  # Веса для
    ...
    last_signal = pd.concat([last_signal, candel_tb[["Open time",'asset', 'signal', 'Удар','sharp','sortino']][-1::]],
